...
export class QuoteDetailController {
  ...
  @Get()
  async findAllQuoteDetails(@Query() variables: any): Promise<QuoteDetailEntity[]> {
  ...
  @Get(':id')
  async findOneQuoteDetail(@Param('id') id: number, @Query() variables: any): Promise<QuoteDetailEntity> {
    const { populate, filters } = this.utilitiesService.handlePopulate(variables);
    return await this.quoteDetailService.findQuoteDetailById(id, populate, filters);
  }

  @Post()
  async createQuoteDetail(@Body() quoteDetail: QuoteDetailEntity): Promise<QuoteDetailEntity> {
    return this.quoteDetailService.createQuoteDetail(quoteDetail);
  }

  @Put(':id')
  async updateQuoteDetail(@Param('id') id: number, @Body() quoteDetail: QuoteDetailEntity): Promise<UpdateResult> {
