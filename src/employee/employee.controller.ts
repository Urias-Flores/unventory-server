...
export class EmployeeController {
  ...
  @Get(':id')
  async getEmployee(@Param('id') id: number, @Query() params: any): Promise<EmployeeEntity> {
    const populate = params.populate ? params.populate.split(',') : [];
    delete params.populate;
  ...
  @Post()
  async createEmployee(@Body() employee: EmployeeEntity): Promise<EmployeeEntity> {
    return this.employeeService.createEmployee(employee);
  }

  @Put(':id')
  async updateEmployee(@Param('id') id: number, @Body() employee: EmployeeEntity): Promise<UpdateResult> {
