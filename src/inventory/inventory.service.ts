...
export class InventoryService {
  ...
  async findAllInventory(populate: [], filters: {}): Promise<InventoryEntity[]> {
    try {
  ...
  async findInventoryById(id: number, populate: [], filters: {}): Promise<InventoryEntity> {
  ...
  async updateInventory(id: number, inventory: InventoryEntity): Promise<UpdateResult> {
    try {
      return await this.inventoryRepository.update({inventoryId: id}, inventory);
    } catch(error) {
      console.log(error);
      throw new HttpException('error when trying to connect to the server', 500);
    }
