...
export class BalanceDetailService {
  ...
  async findAllBalanceDetail(populate: [], filters: {}): Promise<BalanceDetailEntity[]>{
    try {
  ...
  async findBalanceById(id: number, populate: [], filters: {}): Promise<BalanceDetailEntity> {
  ...
  async createBalanceDetail(balanceDetail: BalanceDetailEntity): Promise<BalanceDetailEntity> {
    try {
  ...
  async updateBalanceDetail(id: number, balanceDetail: BalanceDetailEntity): Promise<UpdateResult> {
    try {
      return await this.balanceDetailRepository.update({balanceDetailId: id},balanceDetail);
    } catch(error) {
      console.log(error);
      throw new HttpException('error when trying to connect to the server', 500);
    }
