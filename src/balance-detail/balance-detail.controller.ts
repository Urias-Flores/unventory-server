...
export class BalanceDetailController {
  ...
  @Get()
  async findAllBalanceDetail(@Query() params: String | any): Promise<BalanceDetailEntity[]> {
    const parsedPopulate: [] = params.populate ? params.split(',') : [];
    delete params.populate;
    return await this.balanceDetailService.findAllBalanceDetail(parsedPopulate, params);
  }

  @Get(':id')
  async findBalanceDetailById(@Param('id') id: number, @Query() params: String | any): Promise<BalanceDetailEntity> {
    const parsePopulate: [] = params.populate ? params.populate.split(',') : [];
    delete params.populate;
    return await this.balanceDetailService.findBalanceById(id, parsePopulate, params);
  }

  @Post()
  async createBalanceDetail(@Body() balanceDetail: BalanceDetailEntity): Promise<BalanceDetailEntity> {
    return await this.balanceDetailService.createBalanceDetail(balanceDetail);
  }

  @Put(':id')
  async updateBalanceDetail(@Param('id') id: number, @Body() balanceDetail: BalanceDetailEntity): Promise<UpdateResult>{
